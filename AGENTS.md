...
Karpathy LLM-wiki 对齐要求 TellMe 还应支持探索结果累积：query/output 不应只停留在 terminal 或 `runs/`，有价值的输出应能作为 synthesis/output candidate 回流到 `staging/`，经审核发布到 `vault/`。Obsidian 应逐步成为 raw、compiled graph、review queue、synthesis/output 和健康检查结果的统一 IDE surface。
...
5. 审核 `staging/` 后，由 `tellme --host codex publish --all` 发布 graph node 到 `vault/`。
