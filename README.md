...
- `compile --handoff` 已能要求 Codex 输出 graph candidate JSON，`compile --consume-result` 已能校验候选并把 concept/entity 节点投影到 `staging/`；`publish --all` 可把已审核的 staged graph node 发布到 `vault/`。但还没有自动调用宿主 CLI，也没有冲突解析或复杂 review workflow。
